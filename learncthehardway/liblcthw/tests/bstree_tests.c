...
char *test_traverse()
{
	int rc = BSTree_tranverse(map, traverse_good_cb);
	mu_assert(rc == 0, "Failed to traverse");
	mu_assert(traverse_called == 3, "Wrong count traverse.");

	traverse_called = 0;
	rc = BSTree_tranverse(map, traverse_fail_cb);
...
char *test_fuzzing()
{
	...
	{
		int num = rand();
		number[i] = bformat("%d", num);
		data[i] = bformat("data %d", num);
		BSTree_set(store, number[i], data[i]);
	}

	for(i = 0; i < 100; i++)
	{
		bstring value = BSTree_delete(store, number[i]);
		mu_assert(value == data[i], "Failed to delete the right number.");

		mu_assert(BSTree_delete(store, number[i]) == NULL, "Should get nothing.");

		for(j = i + 1; j < 99 - i; j++)
		{
			bstring value = BSTree_get(store, number[j]);
